...
- **Reference preview/validation/rendering still disagree in legacy video cases.** `prompts.preview_compiled_prompt` and `/generate-video` validation can count an automatic storyboard image, while `_generate_scene_video` treats saved frame slots separately and filters their media out. Consolidate identity/order/count handling before promising identical preview and provider numbering for all inputs. Verified on 2026-09-09 with a stubbed `_generate_scene_video`: for a shot whose video references were **never saved** (`video_references_explicit` false) but whose video prefix list carries the tone item, `/generate-video` overwrites `compiledVideoReferenceItems` with the prefix's resolved list while the render still ships only `defaultVideoReferencePaths` — the prompt calls `图1` the tone sheet, the request carries the storyboard frame as image 1, and the tone sheet is not sent at all. Shots saved from the editor (explicit references) ship prefix-first as previewed. Note also that `video_service.generate_video` hands Gemini only `references[0]` as the image-to-video source, so a prefix-first tone sheet becomes that provider's opening frame; Doubao and Qwen send every image as a reference. Any fix has to choose an order for video (storyboard frame first vs. prefix first) and apply it to preview, batch validation, and render together.
