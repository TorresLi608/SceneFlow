...
LangGraph 已随当前 LangChain 依赖安装，但不作为媒体生产总编排器。采用混合架构：

- LangGraph 适合剧本结构化、角色/地点提取、连续性检查、人工确认与修改后续跑等 LLM 决策流程。
- `generation_jobs + worker` 负责图片、TTS、视频、FFmpeg 等确定性长任务，提供租约、幂等、取消和失败重试。
- “回滚”不删除已生成文件：资产使用版本和当前选择指针；回退阶段只切换指针并将下游标记 stale，需要时重新入队。
- LangGraph checkpoint 只保存小型结构化状态和人工中断点，不保存图片、音频、视频二进制或供应商长任务状态。

这与常见开源短剧项目的多阶段/多 Agent 思路一致，但将不可靠的媒体调用从 Agent 图中剥离，避免进程中断后无法恢复或重复扣费。
