...
- **The agent** — `app/services/agent_service.py` uses LangChain `create_agent` with `generate_image`, `generate_pdf`, and `generate_word_document`. When `SCENEFLOW_SEARXNG_BASE_URL` is configured, it dynamically registers `web_search` backed by SearXNG (with optional `SCENEFLOW_SEARXNG_ENGINE_TOKEN`). It adds read-only `search_error_logs` when the user is a super admin and checks that role again on tool execution. Generated artifacts use signed 30-day links and server-controlled paths. Context assembly is plain Python functions; no durable LangGraph checkpointer or production approval graph is configured.
