...
`data/app.db`, legacy `backend/sceneflow.db`, and `backend/private_generated/` contain real local data. Do not reset, overwrite, or delete them unless explicitly asked. Use temporary `SCENEFLOW_DB_PATH` **and** `SCENEFLOW_PRIVATE_GENERATED_DIR` for checks that import/start the app; clear `DATABASE_URL` or point it at the same temporary database because it takes precedence. The test runner overrides both database variables. Development login defaults to `superAdmin` / `superAdmin@123`; `SCENEFLOW_SUPER_ADMIN_USERNAME` configures the login name.
