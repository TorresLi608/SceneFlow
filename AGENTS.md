...
- Standalone image/video history lives in the backend `generation_records` table (per account, newest 50, served with fresh signed URLs); saved standalone voices live in `user_voices`, isolated from project `voice_profiles` (project design never writes a library row and there is no library import). Neither panel keeps history in localStorage. Super-admin retention is **one window per standalone menu** (`system_settings.generation_retention_{image,video,chat,voice}_days`, 0 = keep forever, edited at `/admin/generation-retention`), applied hourly by `retention_service.RetentionSweeper` in the backend process: image/video rows with their files, chat sessions idle past the window with their messages and `chat/<session>/` tool artifacts, and `user_voices` (drafts and saved) with their audition audio. **Nothing inside an AI 生剧 project (storyboard frames, clips, project voices, assets, tone sheets, exports) is ever subject to retention.** The image/video history, chat session, and voice list responses each carry their own `retentionDays`, and the panels show a cleanup notice when it is nonzero. Reset clears only the current editor/preview and restores initial choices, never history or saved voices. An empty video FPS capability list means omit the parameter, not default to 24.
