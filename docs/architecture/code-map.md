...
| Standalone images / videos | `image-generation-actions.ts`, `video-generation-actions.ts` | `images.py` → `llms/router.py`; `videos.py` → `video_service.py`; history rows via `generation_record_service.py`, history lists carry `retentionDays` for the panels' cleanup notice | `test_images.py`, `test_video_service.py`, `test_generation_records.py` |
| Generation retention (admin) | `admin-actions.ts`, [admin/generation-retention/page.tsx](../../frontend/src/app/(workspace)/admin/generation-retention/page.tsx) | `admin.py` → `system_setting_service.py`; `generation_record_service.py::RetentionSweeper` started in `main.py` lifespan | `test_generation_retention.py` |
...
| [user_voice.py](../../backend/app/models/user_voice.py) | `user_voices` | Standalone voice workspace's account library (draft versus saved); isolated from project `voice_profiles`, with no shared writes or import |
| [generation_record.py](../../backend/app/models/generation_record.py) | `generation_records` | Account-level standalone image/video results: stored path, prompt, model, options; soft-deleted with file removal; hard-deleted by the retention sweeper |
| [system_setting.py](../../backend/app/models/system_setting.py) | `system_settings` | Super-admin runtime policy as key/value; currently `generation_retention_days` (0 = keep forever) |
