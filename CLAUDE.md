...
- **`SQLModel.metadata.create_all()` only creates missing tables.** A new column on an existing table also needs an entry in `_add_missing_columns()` in `app/core/database.py`, or existing databases drift. That file's raw SQL (compat migrations, the legacy `user_configs`/`official_model_configs` → `model_configs` merge) is deliberate — it operates on tables that no longer have models.
