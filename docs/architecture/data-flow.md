...
- `Prop` holds object settings and an optional character owner. `VoiceProfile` holds a project timbre and audition; `UserVoice` is the standalone voice workspace's account library. The two are isolated: project voice design writes no library entry, and there is no import from the library into a project.
...
The project voices page uses `projects-actions.designVoiceProfileAction` → `POST /api/projects/:id/voices/design` → `voice_design` job → `job_handlers.design_voice`. Without a nonempty `voiceId`, success creates a project profile; with one, the API checks project ownership and the handler updates that existing profile. Its ID and order remain unchanged, preserving character bindings. No `UserVoice` row is written in either case; the account library is the standalone `/audio` workspace's alone.
...
Selected characters with drawn sheets are named as references; selected text-only settings are reasoned about from their descriptions; unknown characters are inferred from the script. The merged cast/prop sheets (`project.character_sheet_path` / `prop_sheet_path`) appear as chips beside the cards once drawn and map to `useCastSheet` / `usePropSheet`. Voices are not breakdown inputs: the request has no voice fields and the reference block never mentions timbre. An empty selection is valid. Speaker names/aliases resolve best-effort. Both prompts are normalized to `分镜X：` using `with_shot_label`, and the breakdown instructions request explicit continuity from the previous shot.
...
The manager and reference pickers search names, descriptions, episode titles and label aliases locally; the prompt `@` suggestions and the explicit picker also match the displayed media type (image / video / audio) and source-kind words, show a media badge per asset, and the picker filters by media type with the used/allowed budget per type. Management also filters media, source kind and episode/shared scope. Cards initially render 36 results with “load more”. Metadata is still fetched in full; this is not server pagination. Generated shot labels include episode, shot order and image/video kind. The resolver/compiler accept both these labels and old `@分镜 N` / episode-title labels; ambiguous old duplicate labels cannot reconstruct intent and retain first-reference precedence. Current-shot video self-reference remains excluded. First-frame defaults remain off.
...
Retention is per standalone menu, never per project. `retention_service.run_retention_sweep` reads four windows from `system_settings` (`generation_retention_{image,video,chat,voice}_days`, 0 = keep forever) and, per category in its own transaction: hard-deletes `generation_records` of that kind older than the window and unlinks their files; hard-deletes `chat_sessions` whose `updated_at` (falling back to `created_at`) is older than the window, with their `chat_messages` and the `chat/<session id>/` artifact directory; hard-deletes `user_voices` (drafts and saved) older than the window with their audition WAV. Soft-deleted rows age out on the same clock. The sweeper runs hourly in the backend process and the admin sweep endpoint runs the same pass on demand. Project tables (`scenes`, `episodes`, `voice_profiles`, `assets`, `export_jobs`) and their media are outside every window by design.
