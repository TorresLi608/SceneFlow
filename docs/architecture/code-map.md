...
| Standalone images / videos | `image-generation-actions.ts`, `video-generation-actions.ts` | `images.py` → `llms/router.py`; `videos.py` → `video_service.py`; history rows via `generation_record_service.py` | `test_images.py`, `test_video_service.py`, `test_generation_records.py` |
...
| [generation_record.py](../../backend/app/models/generation_record.py) | `generation_records` | Account-level standalone image/video results: stored path, prompt, model, options; soft-deleted with file removal |
