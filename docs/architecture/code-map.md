...
| `/projects/:projectId/episodes` | [(workbench)/episodes](../../frontend/src/app/projects/[projectId]/(workbench)/episodes/page.tsx) | Episode CRUD, per-episode video composition dialog, and navigation into the editor |
...
| `/projects/:projectId/videos` | [(workbench)/videos](../../frontend/src/app/projects/[projectId]/(workbench)/videos/page.tsx) | Select composed whole-episode videos in order, merge and download delivery exports |
...
| [mention-textarea.tsx](../../frontend/src/app/projects/[projectId]/episode/[episodeId]/_components/mention-textarea.tsx) | `prompt-area` adapter for typed mentions, the searchable type-filtered reference picker with media badges, and reference chips |
...
Shared [episode-video-composer.tsx](../../frontend/src/app/projects/[projectId]/_components/episode-video-composer.tsx) serves the episode list and editor, showing completed shots, numbered selection order, job progress and the composed episode preview.
...
| Project voice design/redesign | [(workbench)/voices/page.tsx](../../frontend/src/app/projects/[projectId]/(workbench)/voices/page.tsx), `projects-actions.ts` | `voices.py`, `voice_service.py`, `qwen_voice_service.py`, `tts_service.py`, `job_handlers.py::design_voice` | `test_voices_api.py`, `test_voice_design_api.py` (including isolation from the account library), `test_qwen_voice_service.py`, `test_tts_service.py` |
| Standalone voice library | [audio/_components/voice-generation-panel.tsx](../../frontend/src/app/(workspace)/audio/_components/voice-generation-panel.tsx), `voice-generation-actions.ts` | `user_voices.py`, `qwen_voice_service.py` | `test_voice_design_api.py` (library listing), `test_qwen_voice_service.py` |
...
| Standalone images / videos | `image-generation-actions.ts`, `video-generation-actions.ts` | `images.py` → `llms/router.py`; `videos.py` → `video_service.py`; history rows via `generation_record_service.py`, history lists carry that kind's `retentionDays` for the panels' cleanup notice | `test_images.py`, `test_video_service.py`, `test_generation_records.py` |
| Generation retention (admin) | `admin-actions.ts`, [admin/generation-retention/page.tsx](../../frontend/src/app/(workspace)/admin/generation-retention/page.tsx) | `admin.py` → `system_setting_service.py` (one window per standalone menu: image, video, chat, voice); `retention_service.py` owns the per-category purges and `RetentionSweeper`, started in `main.py` lifespan; never touches project tables | `test_generation_retention.py` |
| Chat / generated documents | `chat-actions.ts`, `use-chat-controller.ts`, `chat-message-state.ts`, composer and message list | `chat.py`, `chat_service.py`, `graph/graphs/context_graph.py`, `agent_service.py`, `artifact_service.py`, `utils/attachment_parser.py`; web tools `searxng_service.py` (SearXNG search, per-turn budget) and `crawl_service.py` (`fetch_web_content`, public-address checks) | `test_agent_service.py`, `test_chat_balance.py`, `test_artifact_service.py`, `test_searxng_service.py`, `test_crawl_service.py`; frontend `chat-message-state.test.mts` |
...
| [user_voice.py](../../backend/app/models/user_voice.py) | `user_voices` | Standalone voice workspace's account library (draft versus saved); isolated from project `voice_profiles`, with no shared writes or import; hard-deleted with its audition audio by the voice retention window |
| [generation_record.py](../../backend/app/models/generation_record.py) | `generation_records` | Account-level standalone image/video results: stored path, prompt, model, options; soft-deleted with file removal; hard-deleted by the image or video retention window |
| [system_setting.py](../../backend/app/models/system_setting.py) | `system_settings` | Super-admin runtime policy as key/value; currently `generation_retention_{image,video,chat,voice}_days` (0 = keep forever), one per standalone menu |
