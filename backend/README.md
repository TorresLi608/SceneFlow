...
- Video model pricing accepts `unitName: "request"` (one successful generation) or `"second"` (the resolved duration sent to the provider). Standalone and project video calls use the same metering service; usage rows preserve the billed quantity, unit and price snapshot. Existing records are not repriced when a configuration changes.
