...
| BUG-20260922-chat-fetch-web-content-ssrf | 智能问答、fetch_web_content、SSRF、内网探测、follow_redirects、nip.io、0177.0.0.1、assert_public_url、MAX_RESPONSE_BYTES、CRAWL_TOTAL_TIMEOUT_SECONDS、crawl_service | 网页正文提取跟随重定向且不解析 DNS，公网地址跳转或指向回环的域名可读取内网页面，响应无大小/类型限制、无总时长上限；现解析地址逐跳校验、限制 2 MB 文本、整体 30 秒并脱敏日志 | 已修复，单测与本机 DNS 实测通过；DNS 重绑定与 Crawl4AI 远端抓取为已知限制 | 2026-09-22 | [网页正文提取 SSRF 与资源上限](2026-09-22-chat-fetch-web-content-ssrf.md) |
| BUG-20260922-chat-web-search-turn-cache | 智能问答、联网搜索、web_search、SearXNG、searched_cache、对比类问题、MAX_WEB_SEARCHES_PER_TURN、agent_service | 同一轮第二次不同关键词的搜索静默返回第一次的结果导致答非所问；现按关键词缓存、每轮最多 3 次不同搜索，超限返回说明文本；附带精简 SearXNG token 参数并停止记录搜索词 | 已修复，单测回归通过 | 2026-09-22 | [同轮搜索缓存串味](2026-09-22-chat-web-search-turn-cache.md) |
| BUG-20260922-chat-crawl4ai-markdown-shape | 智能问答、fetch_web_content、Crawl4AI、raw_markdown、fit_markdown、字典 repr、crawl_service、priority | Crawl4AI 0.5+ 的 /crawl 把 markdown 序列化为对象，代码按字符串处理把字典 repr 喂给模型；现兼容对象/字符串两种形态并安全读取标题，去掉无效 priority 字段 | 已修复，单测回归通过；真实 Crawl4AI 服务待验证 | 2026-09-22 | [Crawl4AI markdown 对象解析](2026-09-22-chat-crawl4ai-markdown-shape.md) |
| BUG-20260922-chat-web-search-prompt-unconfigured | 智能问答、web_search、SearXNG 未配置、系统提示词、_agent_system_prompt、is_searxng_configured | 未配置 SearXNG 时提示词仍宣称有 web_search 工具，模型会调用不存在的工具或编造搜索；现提示词按配置分支 | 已修复，单测回归通过 | 2026-09-22 | [未配置搜索时的提示词](2026-09-22-chat-web-search-prompt-unconfigured.md) |
| BUG-20260922-chat-search-step-display | 智能问答、执行流程、agent_step、共精选 N 条、检索到 N 条、formatStepDetail、英文界面、extract_crawl_step_summary、chat-message-list | 搜索结果头部计数在截断前生成、抓取完成 detail 是整段 Markdown、英文界面正则与后端文案不匹配；现先裁剪再计数、新增抓取摘要、前端按真实文案翻译 | 已修复，后端单测与前端类型检查/Lint 通过；浏览器未实测 | 2026-09-22 | [搜索步骤计数与文案](2026-09-22-chat-search-step-display.md) |
...
| BUG-20260908-qwen-voice-design-target-model | Qwen、DashScope、qwen-voice-design、target_model、音色设计、qwen_voice_service | 任务模型被误作合成目标；共享适配器增加目标模型回退，账号音色 targetModel 元数据仍待对齐（2026-09-18 起项目任务不再写入账号音色库，仅剩独立音色页一处） | 本地回归通过，实网待验证 | 2026-09-18 | [Qwen 音色目标模型](2026-09-08-qwen-voice-design-target-model.md) |
