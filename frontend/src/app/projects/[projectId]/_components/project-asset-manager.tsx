...
import type { Asset, GenerationReferenceKind } from "@/types/project";
...
type ManagedAsset = Asset & { referenceKind: GenerationReferenceKind; episodeId: string | null; episodeTitle: string };

export function ProjectAssetManager({ projectId, open, onOpenChange, onChanged }: {
  projectId: string;
  open?: boolean;
  ...
  const catalog = useProjectResources(projectId);
  const [search, setSearch] = useState("");
  const [source, setSource] = useState("all");
  const [episodeFilter, setEpisodeFilter] = useState("all");
  const [visibleCount, setVisibleCount] = useState(36);
  const [editing, setEditing] = useState<ManagedAsset | null>(null);
  const changed = () => {
    for (const queryKey of [queryKeys.projectResources(projectId), queryKeys.assets(projectId), queryKeys.episodes(projectId), queryKeys.characters(projectId), queryKeys.props(projectId), queryKeys.voices(projectId), queryKeys.projects]) {
      void queryClient.invalidateQueries({ queryKey });
    }
    void queryClient.invalidateQueries({ queryKey: ["episode", projectId] });
    onChanged?.();
  };
  ...
  const filtered = resources.filter((item) => matchesResource(item, search) &&
    (source === "all" || item.kind === source) &&
    (episodeFilter === "all" || (episodeFilter === "shared" ? !item.episodeId : item.episodeId === episodeFilter)));
  const visible: ManagedAsset[] = filtered.filter((item) => item.media === tab).map((item) => ({
    id: item.id, projectId, name: item.label, description: item.description, kind: item.media,
    mediaType: item.media, url: item.url, createdAt: null, updatedAt: item.updatedAt,
    referenceKind: item.kind, episodeId: item.episodeId, episodeTitle: item.episodeTitle,
  }));
  const counts = Object.fromEntries(["image", "video", "audio"].map((media) => [media, resources.filter((item) => item.media === media).length]));
  const episodes = [...new Map(resources.filter((item) => item.episodeId).map((item) => [item.episodeId!, item])).values()].sort((a, b) => (a.episodeNumber ?? 0) - (b.episodeNumber ?? 0));
  const mergeOptions = filtered.filter((item) => item.media === "image").map((item) => ({
    id: item.kind === "asset" ? item.id : `${item.kind}:${item.id}`, name: item.label,
  }));
  ...
  const startEdit = (asset: ManagedAsset) => {
    setEditing(asset);
    setName(asset.name);
  ...
  const confirmDelete = async () => {
    ...
    setDeleting(true);
    try {
      if (pendingDelete.referenceKind === "asset") await deleteAssetAction(projectId, pendingDelete.id);
      else await deleteGenerationReferenceAction(projectId, pendingDelete.referenceKind, pendingDelete.id);
      setSelected((ids) => ids.filter((id) => id !== (pendingDelete.referenceKind === "asset" ? pendingDelete.id : `${pendingDelete.referenceKind}:${pendingDelete.id}`)));
            ...
            <div className="flex flex-col min-h-0 rounded-xl border border-border/60 bg-muted/10 p-3.5 overflow-hidden">
              ...
              {visible.length === 0 ? (
                <div className="flex flex-1 flex-col items-center justify-center gap-2 p-8 text-center text-muted-foreground">
                  ...
                  <p className="text-xs">{t(search || source !== "all" || episodeFilter !== "all" ? "assets.noMatches" : "episode.assetEmpty")}</p>
                ...
                <div className="grid min-h-0 flex-1 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 overflow-y-auto pr-1 content-start auto-rows-max chat-message-list-scrollbar">
                  {visible.slice(0, visibleCount).map((asset) => (
                    ...
                    >
                      ...
                      <button
                        type="button"
                        aria-label={t("episode.assetClickPreview")}
                        onClick={() => {
                      ...
                      {asset.kind === "audio" && asset.url ? <audio controls preload="none" src={asset.url} className="mt-2 w-full" aria-label={asset.name} /> : null}

                      {/* Info & Actions */}
                      <div className="mt-2.5 flex items-start justify-between gap-2">
                        <div className="min-w-0 flex-1">
                          <p title={asset.name} className="text-xs font-semibold text-foreground break-words">
                            {asset.name}
                          </p>
                          <p className="mt-1 text-[11px] text-muted-foreground">{t(`assets.kind.${asset.referenceKind}`)} · {asset.episodeId ? asset.episodeTitle : t("assets.shared")}</p>
                          {asset.episodeId ? <Link className="text-[11px] text-primary underline" href={`/projects/${projectId}/episode/${asset.episodeId}`}>{t("assets.openEpisode")}</Link> : null}
                          {asset.description ? (
                        ...
                        <div className="flex items-center gap-0.5 shrink-0">
                          {asset.referenceKind === "asset" ? <Button
                            size="icon-xs"
                            variant="ghost"
              ...
              {visible.length > visibleCount ? <Button variant="outline" onClick={() => setVisibleCount((count) => count + 36)}>{t("assets.loadMore")}</Button> : null}
            </div>
            ...
            <div className="flex flex-col min-h-0 gap-4 overflow-y-auto pr-1 chat-message-list-scrollbar">
              ...
              {tab === "image" && !editing ? (
                <div className="flex flex-col gap-3 rounded-xl border border-border/70 bg-card/60 p-4 shadow-sm">
                  ...
                  <div className="flex max-h-40 flex-col gap-1.5 overflow-y-auto rounded-lg border border-border/50 bg-muted/30 p-2 text-xs chat-message-list-scrollbar">
