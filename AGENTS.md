...
- Standalone image/video history lives in the backend `generation_records` table (per account, newest 50, served with fresh signed URLs); saved standalone voices live in `user_voices`, isolated from project `voice_profiles` (project design never writes a library row and there is no library import). Neither panel keeps history in localStorage. A super-admin retention policy (`system_settings.generation_retention_days`, 0 = keep forever, edited at `/admin/generation-retention`) is applied hourly by `RetentionSweeper` in the backend process to `generation_records` only; the history list responses carry `retentionDays`, and the image/video panels show a cleanup notice when it is nonzero. Reset clears only the current editor/preview and restores initial choices, never history or saved voices. An empty video FPS capability list means omit the parameter, not default to 24.
