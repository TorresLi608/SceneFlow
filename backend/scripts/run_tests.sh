...
# data/app.db or backend/sceneflow.db is never touched.
FAILED=""
for name in "$@"; do
  DB=$(mktemp -u /tmp/sf_test_XXXXXX.db)
  if DATABASE_URL="sqlite:///$DB" SCENEFLOW_DB_PATH="$DB" PYTHONPATH=. .venv/bin/python "tests/$name.py" >"/tmp/$name.log" 2>&1; then
    echo "PASS  $name"
  else
    echo "FAIL  $name"
    FAILED="$FAILED $name"
