...
- **Both prompts open with `分镜X：`, and the number is re-derived on write.** `BREAKDOWN_SYSTEM` asks the model to label every `visualPrompt` and `videoPrompt` with the shot's number and — from the second shot on — to follow it with a `【承接…】` block naming what carries over from the previous shot (scene, wardrobe, pose, position, light). Continuity was previously only a vague instruction, and the motion prompts ignored it. The number itself is never trusted from the model: `_shot_values` runs every prompt through `prompt_service.with_shot_label(text, order)`, which strips a missing or wrong label and re-applies one from `order_num`, so shots inserted, deleted or reordered afterwards stay correctly numbered. `target: "video"` is also shown each existing shot's frame prompt, not just its narration — a 承接 block cannot be written against a one-line summary.
...
- **A successful tone sheet writes a prefix prompt into every shot of the episode**, pointing each at its own numbered cell in the grid and at the cells either side — which is only expressible because the anchor is one image containing every shot. Written server-side in `storyboard_service._write_tone_prefixes` rather than left to the editor, because the text depends on the shot's position in a grid that does not exist until the sheet does. Applied to the whole episode even when the run rendered a subset, or an excluded shot would be the only one without the context. Regenerating rewrites the item in place (a stable `prefix-tone-{order}` id) instead of stacking a second copy on a shot the user has edited around; the write is best-effort, since a shot missing its preamble still renders and failing the run would discard a sheet already paid for. A user who deletes the item can get the identical wording back from `GET /api/prompts/prefix-presets` — served, not templated in the browser, so the two can never drift.
...
- The storyboard image is the automatic first-frame reference only when the selected model supports first frames. Wan 2.7 advertises first-frame only; Wan 3.0 advertises first and last frames. The editor places first/last-frame selectors below the video prompt, while image/video/audio mentions remain additional references. **A frame slot is not a reference**: the renderer passes it as `first_frame`/`last_frame` and strips it back out of the reference list, so it is never one of the numbered `图N` the prompt can talk about. The editor therefore never adds a chosen frame to the shot's reference chips or writes an `@mention` for it — doing both spent two slots on one image and put `@分镜 N` (the shot's own frame) into its own motion prompt on every reload. Additional references are resolved back to stored paths under the same project and capped by `videoCapabilities`. When either a first or last frame is used and the model supports `adaptive`, the renderer overrides the configured ratio to `adaptive`, matching the provider recommendation. Required reference kinds disable generation until satisfied; text-to-video models do not require a frame first.
- **Turning a frame slot off has to be storable.** The editor suggests the shot's own render for the first frame, so "off" and "not yet decided" must not look alike: `videoFirstFrame: ""` clears the slot (a JSON `null` would read as "leave alone"), the column keeps a stored `null`, and `videoFirstFrameExplicit` tells the editor a decision was made so it stops re-suggesting.
...
- **Prefix prompts (前置提示词) are part of the prompt, and their mentions are part of the budget.** Each prompt carries an ordered list of `{id, name, prompt, references}` preambles (`image_prompt_prefixes_json` / `video_prompt_prefixes_json`) concatenated ahead of it. They live beside the prompt rather than inside it so re-running the breakdown, which rewrites `visual_prompt` and `video_prompt` wholesale, cannot take the episode-level context with it. `prompt_prefix_service.combined_references` folds the preamble's `@素材` and the shot's own into one deduplicated, prefix-first list before anything is resolved — the same order `combined_prompt` concatenates the text in, and therefore the order `compile_prompt` numbers `图1`, `图2`… in. An asset named in both a preamble and the prompt is one reference with one number, not two slots. `/api/prompts/compile` takes the raw `prefixes` and combines them server-side for exactly this reason: a client that pre-concatenated would preview a numbering the render does not use.
