...
A `Project` is a **series** and owns no shots directly. Content hangs off `Episode`, and each `Scene` is one shot inside an episode. `Character` + `CharacterVariant` form the series bible that keeps a cast member's look, image model, and voice stable across episodes; `SceneCharacter` records who appears in a shot. Shot order restarts at 1 per episode, so a serialized project carries **one** episode's shots. Full detail in `data-flow.md` and the root `CLAUDE.md`.
